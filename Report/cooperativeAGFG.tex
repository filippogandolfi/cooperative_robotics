...
<<<<<<< Updated upstream
=======
>>>>>>> parent of c3c9316... Eqaution exercise 3 done
...
By default the \textit{Horizontal Attitude} is enable and it has higher priority than the \textit{Vehicle Position} task. This results in the UVMS trying to keep a horizontal orientation with respect to the sea floor (Figure  \ref{fig:hae}): if the target position has pitch and roll different from zero, the robot doesn’t perform the vertical position shown in (Figure  \ref{fig:hane}).
When the \textit{Horizontal Attitude} is not enable, the UVMS try to find the alignment between its frame and the goal one, without taking (of course) into account to stay in parallel with the seafloor. If the target position has pitch or roll, the robot will try to perform them.
...
So, if we switched the position we will have the same performance as if we disable the \textit{Horizontal Attitude} task.\\
In the end, it's not usefull to switch the position and it's wrong because the \textit{Horizontal Attitude} is a safety task, so it's, every time, more important (so high priority) than a action-defining task. 
...
If the \textit{Tool Position} task is enable, the vehicle could not reach the desired position. If it's enable ad it as higher priority than the \textit{Vehicle Position} task, the robot try to achieve the goal position for the arm, not the ones referred to the vehicle it self. \\
...
However, we continue to use the \textit{Manipulability} task, in order to maintain a safe position for the arm. 
<<<<<<< HEAD
=======
...
\textbf{Minimum Altitude} [R,I,S], it has high priority because it's a safety task, UVM must avoid crashes with the seafloor.
<<<<<<< HEAD
=======
...
\subsubsection{Q3: how does this task differs from a minimum altitude control task?}
There are two major difference, the first one is that this task it isn't a safety task, it's a Action-Defining task. The \textit{Minimum Altitude} it's used to avoid crash with the seafloor, the \textit{Landing} task it's like the Vehicle Position task, and, as this one, it has low priority.
...
%\dot{\overline{x}}_{jointLimit,i} = k
%\begin{cases}
