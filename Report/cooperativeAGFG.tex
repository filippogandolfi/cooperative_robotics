...
The \textit{Vehicle Null} only puts vehicle velocities to zero, therefore it is not able to compensate disturbances. In a similar scenario, the robot will be influenced by the currents: if the disturbance is not too big, the robot is able to arrive to the desired position and to land. However, when the \textit{Vehicle Null} task is triggered in Action D, the UVMS will start drifting, losing the target. The arm will keep trying to reach the desired position stretching as much as possible.


With a check on the position error during the mission phase update, it is possible to return to Action A in order to achieve again the desired position.
