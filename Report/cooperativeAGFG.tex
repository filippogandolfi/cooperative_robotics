...
\large
 ...
 & \underset{3\times3}{\boldsymbol{0}_{ }} & \underset{3\times3}{^{w}\boldsymbol{R}_{v}} \\
\underset{6\times7}{\boldsymbol{0}} \\
& \underset{ 3\times 3}{^{w}\boldsymbol{R}_{v}} & \underset{3\times 3}{\boldsymbol{0}_{ }} \\
\end{bmatrix}
\end{equation}
\\ 
...
\begin{figure}[!htb]
\centering
\includegraphics[width=.5\textwidth]{112_HAenabled.png}\caption{\textit{Horizontal Attitude} enabled, the UVMS is parallel with the seafloor.}
\label{fig:hae}
\vspace{5px}
\centering
\includegraphics[width=.5\textwidth]{112_HANotenabled.png}\caption{\textit{Horizontal Attitude} not enabled, the UVMS is not parallel with the seafloor.}
\label{fig:hane}
\end{figure}
%%%%%%% %%%%%%%%%% %%%%%%% 
...
\end{bmatrix}
    \begin{bmatrix}
     & \underset{3\times 3}{\boldsymbol{0}_{ }} & \underset{ 3\times 3}{^{w}\boldsymbol{R}_{v}} \\
...
\begin{figure}[t]
    ...
    \caption{Minimum altitude is enabled, the UVMS is following the seafloor}
    \label{images_2_1}
\end{figure}
...
\begin{itemize}
    \item \textbf{10 m:} The UVMS has an initial position below this threshold, thus it starts floating towards the surface very quickly (it is possible to notice this vertical velocity $\dot{z}$, in (Figure \ref{10m_ppdot})). 
...
\hfill
    ...
    \caption{Landing activation plot}
    \end{minipage}
\end{figure}
%%%%%%%% MINI PAGE IMAGES %%%%%%%%
...
\begin{description}
\item Action A: the UVMS performs the safe waypoint navigation. All the safety tasks are enabled. This Action finishes when the position error is below a fixed threshold.
...
\begin{description}
 \item Action A, as before, is safe waypoint navigation with all the safety tasks enabled (Figure \ref{311a}). This Action finishes when the position error is below a fixed threshold.
 \item Action B is the action that performs the alignment to the rock, with all the safety task enabled (Figure \ref{311b}). his Action finishes when the misalignment error is below a fixed threshold.
 \item Action C performs the safe landing on the seafloor. \textit{Alignment to the Rock} is gradually disabled to optimize the alignment while the vehicle is performing the landing (Figure \ref{311c}). \end{description}
...
By looking at the first term of the sum, we know that ${^w}\boldsymbol n_{\rho}$ is the misalignment versor projected on the world and $\dot\theta$ can be written as :
...
\begin{description}
 \item Action A, safe waypoint navigation with all the safety task enabled. This Action finishes when the position error is below a fixed threshold (in this case $0.1m$).
 \item Action B, alignment to the nodule with all the safety task enabled. This Action finishes when the misalignment error is below a fixed threshold (in this case $0.07m$).
 \item Action C, landing and smooth rotation to be aligned with the rock. This Action finishes when the vehicle touches the seafloor (in the simulation this happens at $\approx 0.17m$). 
 \item Action D, manipulator actuation after landing. In this Action \textit{Vehicle Null Velocity} task is enabled, preventing vehicle movements. The only movement will be the extension of the manipulator to reach the desired target tool position.
\end{description}
The transitions between Actions are visibile in the Figure \ref{fig:411_errors}.
%%%%%%% near images %%%%%%% 
\begin{figure}[htp]
\centering
...
Since we do not want the vehicle to move, the \textbf{task reference} is a zero vector defined as: 

\begin{equation}
\large
   \dot{\overline{\boldsymbol{x}}}_{vNull} =  \begin{bmatrix} 0 & 0 & 0 & 0 & 0 & 0 \end{bmatrix}^\top
\end{equation}
...
\begin{description}
\item \textbf{Joint Limit} [R, I, S]: it is a safety task, thus it has higher priority than the other action-defining tasks. It is able to control that the joints don't operate over their fixed threshold.
\end{description}
...
\begin{equation}
\large
   \dot{\overline{\boldsymbol{x}}}_{\operatorname{prefShape}} = k(\boldsymbol x_{\operatorname{prefShape}} - \boldsymbol x_{q}) %delta
\end{equation}
where:
\begin{itemize}
    \item $k$ is the control gain.
    \item $\boldsymbol x_{\operatorname{prefShape}}$ is the desired configuration of the first four joints.
    \item $\boldsymbol x_{q}$ is the actual configuration of the first four joints.
\end{itemize} 
...
In the previous simulation, it was not possible to constrain the vehicle velocities to zero, in order to allow a safe manipulation. By adding a safe navigation phase, it possible to move the UVMS by using the \textit{Vehicle Position} instead of using the \textit{Tool} task. In the second phase, the \textit{Tool} task is triggered, but the vehicle velocities are constrained to zero: the manipulator is able to safely reach the goal frame. Moreover, this results in a smoother behaviour as it is shown in the figures below (Figure \ref{522_n1} and \ref{522_n2}).
...
\begin{enumerate}
\item TPIK1 is the same as the previous exercises. It considers the vehicle and the manipulator as a fully controllable system. When the hierarchy of task corresponding to the action $A$ is solved, we take the reference for the vehicle velocities while we discard the joints velocities one.
\item TPIK2 is equal to TPIK1 plus an additional task at the top of the hierarchy.\\
\textbf{Constrain Vehicle Velocities} [NR, E, C], this task is a non-reactive one and it is used to constrain the vehicle velocities to coordinate the arm and vehicle movements.\\

\vspace{5px}
The \textbf{Jacobian} for this task is:
\begin{equation}
\large
...
Therefore, TPIK2 provides the optimal joints velocities by taking into account the "real" movement of the vehicle, which is different from the one computed by the algorithm because of the sinusoidal disturbance (without TPIK2 images: \ref{JL_wout_TPIK2}, \ref{Error_wout_TPIK2}, \ref{zoomed_error_wout_TPIK2}, \ref{Jqandqdot_wout_TPIK2}; with TPIK2 images: \ref{JL_w_TPIK2}, \ref{Error_w_TPIK2}, \ref{zoomed_error_w_TPIK2}, \ref{Jqandqdot_w_TPIK2}).
\end{enumerate}
