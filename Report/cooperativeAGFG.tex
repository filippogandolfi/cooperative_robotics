...
In this specific case the UVMS goal is to align the end-effector frame to the goal frame which is located slightly above the pipe. Since no explicit goal for the vehicle was given, we have used just the tool task (which is able to move the vehicle as well) in order reach the goal directly. Nevertheless, since we have just one mission phase the end-effector moves in order to achieve the goal as soon as possible, leading to a "stretched" shape (FIGURE 32). If we add the preferred shape constraint as defined in the above mentioned task, we can maintain the desired end-effector configuration, even with just one mission phase (FIGURE 33). As we can see in FIGURE 34 e 35, when the preferred shape task is active the joints are in a more natural shape, which is also further from the joint limits.
%%%%%%%% MINI PAGE IMAGES %%%%%%%%
\begin{figure}[htp]
    \centering
    \begin{minipage}{0.50\textwidth}
